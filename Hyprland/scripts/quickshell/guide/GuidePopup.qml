            ...
            Item {
                ...
                ColumnLayout {
                    ...
                    ListView {
                        ...
                        model: ListModel {
                             ListElement { version: "v1.7.89"; title: "Bugfix — face icon cache re-validate, SDDM avatar fallback, install/restore self-deploy"; desc: "Stale face icon cache no longer breaks avatar; SDDM .face fallback now reachable; install.sh/restore.sh now self-update"; icon: "󰒃"; clr: "red"; detail: "FIXED: GuidePopup face icon cache now re-validates the cached path — if the cached file no longer exists, it rescans all face icon locations (was stuck on a deleted ~/.face.icon). FIXED: SDDM avatar fallback — the final .face fallback was blocked by an incorrect condition (else if source !== '.face' — never true when source IS .face). Now uses indexOf('.face.icon') so fallthrough LoginScreen → .face.icon → .face works correctly. FIXED: install.sh and restore.sh now self-deploy during updates — they live outside Hyprland/scripts/ so rsync missed them. Users stuck on old install.sh versions will now get automatic updates. FIXED: Deployed restore.sh per-username face icon (${CURRENT_USER}.face.icon) replaced with generic .face.icon in SDDM fallback chain." }
