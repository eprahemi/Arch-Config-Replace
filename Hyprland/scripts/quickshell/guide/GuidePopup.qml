...
Item {
    ...
    Process {
        ...
        command: [
            ...
            "ICON=\"\"; " +
            "if [ -f \"$CACHE\" ]; then " +
            "  ICON=$(cut -d'|' -f7 < \"$CACHE\" 2>/dev/null); " +
            "  if [ -n \"$ICON\" ] && [ ! -f \"$ICON\" ]; then ICON=\"\"; fi; " +
            "fi; " +
            "if [ -z \"$ICON\" ]; then " +
            "  FACES_DIR=\"$HOME/.config/hypr/Faces\"; if [ -f \"$FACES_DIR/.face.icon\" ]; then ICON=$(readlink -f \"$FACES_DIR/.face.icon\"); elif [ -f ~/.face.icon ]; then ICON=$(readlink -f ~/.face.icon); elif [ -f ~/.face ]; then ICON=$(readlink -f ~/.face); fi; " +
                    ...
                    Item {
                        ...
                        property var tips: [
                            ...
                            "Drop images in ~/.config/hypr/Lockscreen Wallpaper/ to change the lock screen",
                             "Run 'sudo cp your-wallpaper.png /usr/share/sddm/themes/matugen-minimal/wallpaper.png' to change the login screen wallpaper",
            ...
            Item {
                ...
                ColumnLayout {
                    ...
                    ListView {
                        ...
                        model: ListModel {
                             ListElement { version: "v1.7.99"; title: "Caelestia-style Thunar GTK theming + dynamic matugen colors"; desc: "Papirus-Dark icons, adw-gtk3-dark theme, thunar.css with matugen dynamic colors, GTK4 CSS replaced with matugen-based dynamic file"; icon: "󰌊"; clr: "green"; detail: "ADDED: Caelestia-style Thunar GTK theming — Papirus-Dark icon theme installed and set system-wide, adw-gtk3-dark GTK theme (already present), and thunar.css rewritten to use dynamic matugen wallpaper colors instead of hardcoded Catppuccin. UPDATED: gtk.css now imports both matugen/colors-gtk.css and thunar.css for a unified pipeline. REPLACED: GTK4 CSS — removed the 8500-line hardcoded Catppuccin file (backed up as .bak) with a minimal ~80-line dynamic file that imports matugen colors. The wallpaper-change pipeline (matugen → colors-gtk.css → matugen_reload.sh toggle) already handles auto-regeneration — GTK apps update instantly when wallpaper changes." }
                             ListElement { version: "v1.7.98"; title: "Fix: install.sh DOTS_VERSION stuck at 1.7.93 + GuidePopup changelog added"; desc: "install.sh was reporting v1.7.93 instead of v1.7.97; GuidePopup now has all 5 missing changelog entries"; icon: "󰒃"; clr: "red"; detail: "FIXED: install.sh DOTS_VERSION was hardcoded at 1.7.93 — all v1.7.94 through v1.7.97 commits were pushed but the version string in install.sh was never bumped. Friend running the one-liner saw '1.7.93' regardless of latest release. FIXED: GuidePopup.qml missing changelog entries for v1.7.93, v1.7.94, v1.7.95, v1.7.96, and v1.7.97 — now added to the hardcoded ListModel." }
                             ListElement { version: "v1.7.97"; title: "Bump version 1.7.96 → 1.7.97"; desc: "Version bump only"; icon: "󰋼"; clr: "blue"; detail: "Bump version 1.7.96 → 1.7.97." }
                             ListElement { version: "v1.7.96"; title: "Fix: install.sh cp -f → cp -rf for config/ directory"; desc: "cp -f without -r silently skipped the entire Hyprland/config/ folder, causing globbing errors"; icon: "󰒃"; clr: "red"; detail: "FIXED: install.sh used cp -f (without -r) to deploy the Hyprland/config/ directory, which silently skipped it. All config/*.conf files (monitors.conf, env.conf, autostart.conf, etc.) were never deployed. Users got 'globbing error found no match' from Hyprland sourcing ~/.config/hypr/config/*.conf. Changed to cp -rf so the directory and its contents are correctly copied." }
                             ListElement { version: "v1.7.95"; title: "Bump version 1.7.94 → 1.7.95"; desc: "Version bump only"; icon: "󰋼"; clr: "blue"; detail: "Bump version 1.7.94 → 1.7.95." }
                             ListElement { version: "v1.7.94"; title: "Fix: before_sleep_cmd blocking suspend + wake sources disabled"; desc: "Quickshell Lock.qml in before_sleep_cmd held hypridle's sleep inhibitor forever — system never suspended. USB/PCIe wake sources disabled for stable s2idle."; icon: "󰒃"; clr: "red"; detail: "CRITICAL FIX: before_sleep_cmd changed from quickshell Lock.qml (persistent process that never exits — hypridle holds sleep inhibitor forever) to loginctl lock-session (returns immediately, inhibitor released). Root cause of 'laptop not suspending'. FIXED: lock timeout listener also uses loginctl lock-session instead of quickshell directly — consistent with before_sleep_cmd. CHANGED: suspend timeout reduced from 66min to ~43min (10 min after 33min lock). FIXED: Disabled XHC (USB), RP05 (Ethernet), RP06 (WiFi), RP09 (SD card) wake sources in /proc/acpi/wakeup for stable s2idle — was waking immediately from USB/PCIe events." }
                             ListElement { version: "v1.7.93"; title: "CRITICAL: swayidle → hypridle-git — fix -w block, force s2idle, polkit rules"; desc: "swayidle -w blocked suspend because Lock.qml never exits. hypridle-git fixes it. Deep S3 broken — s2idle forced via kernel param."; icon: "󰒃"; clr: "red"; detail: "CRITICAL FIX: swayidle removed — replaced with hypridle-git from AUR. swayidle's -w flag blocked 66min suspend because Lock.qml never exits, -w waits for it before running subsequent timeouts. CRITICAL FIX: hypridle.conf now ACTIVE — handles lock before sleep, idle timeout lock (33min), and idle timeout suspend (66min) via two independent listeners (hypridle does NOT hold inhibitors for lock_cmd, but DOES hold them for before_sleep_cmd — both bugs fixed). CRITICAL FIX: Deep S3 (mem) broken on Lenovo laptops — immediate wake in ~2.8s. s2idle forced via mem_sleep_default=s2idle kernel parameter. FIXED: USB wake enabled on s2idle (XHC toggled in /proc/acpi/wakeup). FIXED: Polkit rule recreated for loginctl lock-session without password. CHANGED: install.sh — swayidle removed from pacman install list, hypridle-git installed from AUR, hypridle.conf deployed." }
                             ListElement { version: "v1.7.92"; title: "SDDM uses system paths — LoginScreen Settings dropped, multi-format wallpaper restored"; desc: "SDDM theme now reads wallpaper/face directly from /usr/share/sddm/ (no more home-dir paths sddm can't access)"; icon: "󰒃"; clr: "red"; detail: "FIXED: SDDM login screen wallpaper and face icon now use system paths (/usr/share/sddm/themes/matugen-minimal/, /usr/share/sddm/faces/) instead of ~/.config/hypr/LoginScreen Settings/ which SDDM could never access (runs as sddm user, home dir is 700). FIXED: bgWallpaper Image visible=true so FastBlur can grab pixels (was false, causing blank login). FIXED: Multi-format wallpaper fallback restored (png/jpg/jpeg/webp/bmp) pointing to theme directory, with background.png as final fallback. FIXED: Avatar Image block had collapsed one-line QML syntax — properly formatted now. UPDATED: LoginScreen Settings path removed from SDDM theme entirely — users place custom files with sudo cp to /usr/share/sddm/ directly." }
                             ListElement { version: "v1.7.91"; title: "SDDM paths switched to absolute — file:/// URLs removed"; desc: "Wallpaper and avatar now use plain absolute paths instead of file:/// URLs that broke on some systems"; icon: "󰒃"; clr: "red"; detail: "FIXED: SDDM wallpaper and avatar paths changed from file:/// URLs to absolute paths (e.g., /home/user/... instead of file:///home/user/...). file:/// URLs with spaces or percent-encoding can fail on some Qt/SDDM versions. Plain absolute paths work universally and avoid URL encoding issues entirely. FIXED: LoginScreen Settings wallpaper and face icon re-deployed with proper files." }
                             ListElement { version: "v1.7.90"; title: "User files preserved — never overwrite photos in override dirs"; desc: "LoginScreen Settings, Lockscreen Wallpaper, Faces dirs now safe from overwrite; system SDDM dirs always updated"; icon: "󰋼"; clr: "blue"; detail: "FIXED: User override directories (~/.config/hypr/Faces/, LoginScreen Settings/, Lockscreen Wallpaper/) — install.sh and restore.sh now NEVER overwrite existing user files. If a user has placed custom wallpapers or face icons in these directories, updates will leave them untouched. FIXED: System SDDM directories (/usr/share/sddm/faces/, /usr/share/sddm/themes/matugen-minimal/) — now always overwritten with latest defaults on every update. FIXED: install.sh now also deploys face icon to /usr/share/sddm/faces/ (was missing entirely — SDDM avatar would fail when LoginScreen Settings was empty). FIXED: restore.sh SDDM theme deploy no longer skips if directory already exists — always updates to latest." }
                             ListElement { version: "v1.7.89"; title: "Bugfix — face icon cache, SDDM avatar/wallpaper, install/restore self-deploy"; desc: "Face icon cache re-validates; SDDM .face fallback, wallpaper fallback fixed; install.sh/restore.sh now self-update"; icon: "󰒃"; clr: "red"; detail: "FIXED: GuidePopup face icon cache now re-validates the cached path — if the cached file no longer exists, it rescans all face icon locations (was stuck on a deleted ~/.face.icon). FIXED: SDDM avatar fallback — the final .face fallback was blocked by an incorrect condition (else if source !== '.face' — never true when source IS .face). Now uses indexOf('.face.icon') so fallthrough LoginScreen → .face.icon → .face works correctly. FIXED: SDDM wallpaper fallback — config.background returns raw 'wallpaper.png' from theme.conf, a relative path that fails in plain Image. Replaced with fully-qualified file:/// URL to theme directory. Also percent-encoded spaces in file:/// paths to %20 for RFC 3986 compliance. FIXED: install.sh and restore.sh now self-deploy during updates — they live outside Hyprland/scripts/ so rsync missed them. Users stuck on old install.sh versions will now get automatic updates. FIXED: Deployed restore.sh per-username face icon (${CURRENT_USER}.face.icon) replaced with generic .face.icon in SDDM fallback chain." }
                             ListElement { version: "v1.7.88"; title: "Multi-format SDDM wallpaper + GuidePopup tip fix + website sync"; desc: "Wallpaper supports any format (png/jpg/jpeg/webp/bmp), GuidePopup tip updated, website stale paths fixed"; icon: "󰋼"; clr: "purple"; detail: "CHANGED: SDDM wallpaper now supports any image format (PNG, JPG, JPEG, WEBP, BMP) in ~/.config/hypr/LoginScreen Settings/Wallpaper/ — sequential fallback with counter-based QML binding. CHANGED: GuidePopup System tab tip updated — now points to ~/.config/hypr/LoginScreen Settings/faces/ override folder instead of /usr/share/sddm/faces/. FIXED: Repo SDDM Main.qml now has multi-format wallpaper fallback (was limited to wallpaper.png only). FIXED: Website FAQ stale paths (4 pages, ~20 outdated references) corrected — ~/.Wallpapers/ → ~/.config/hypr/Lockscreen Wallpaper/, SDDM instructions updated, 16 missing changelog entries added. Version bumped to 1.7.88." }
                             ListElement { version: "v1.7.87"; title: "SDDM LoginScreen Settings — user override folder for face + wallpaper"; desc: "Drop wallpaper.png and .face.icon in ~/.config/hypr/LoginScreen Settings/ to customize the login screen"; icon: "󰋼"; clr: "purple"; detail: "NEW: SDDM theme now reads from ~/.config/hypr/LoginScreen Settings/ FIRST before falling back to system SDDM paths. Place wallpaper.png in LoginScreen Settings/Wallpaper/ to override the login wallpaper. Place .face.icon in LoginScreen Settings/faces/ to override the login avatar. No sudo required — just drop files in your hypr config folder. CHANGED: Avatar fallback chain: LoginScreen Settings → /usr/share/sddm/faces/.face.icon → /usr/share/sddm/faces/.face. CHANGED: Wallpaper fallback chain: LoginScreen Settings → SDDM theme default. CHANGED: install.sh and restore.sh deploy default wallpaper/face to LoginScreen Settings on every update so users always have a baseline to override." }
                            ListElement { version: "v1.7.86"; title: "Face icon deploy policy — only to ~/.config/hypr/Faces/ + restore.sh cleanup"; desc: "Face icon now deployed only to hypr directory, home fallback is manual; stale username-specific refs removed from restore.sh"; icon: "󰋼"; clr: "blue"; detail: "CHANGED: Face icon deployed only to ~/.config/hypr/Faces/.face.icon — ~/.face.icon and ~/.face are now manual-only fallbacks (QML still reads them if primary is missing). FIXED: restore.sh condition and summary still referenced stale /usr/share/sddm/faces/${CURRENT_USER}.face.icon — now uses .face.icon throughout. FIXED: restore.sh face icon block split into two separate blocks: first deploys to ~/.config/hypr/Faces/, second deploys SDDM icon. FIXED: install.sh no longer copies face icon to ~/.face.icon." }
                            ListElement { version: "v1.7.85"; title: "SDDM face icon: reads .face.icon (generic) with .face fallback"; desc: "No more per-username face files — single .face.icon works regardless of system username"; icon: "󰋼"; clr: "blue"; detail: "FIXED: SDDM theme now reads /usr/share/sddm/faces/.face.icon (generic) with fallback to /usr/share/sddm/faces/.face on error (via onStatusChanged: Image.Error). No more per-username files — users place a single file regardless of system username. CHANGED: restore.sh copies to /usr/share/sddm/faces/.face.icon only (removed username variant). SDDM avatar in Main.qml updated in both repo copy and deployed system copy." }
                            ListElement { version: "v1.7.84"; title: "Bug fixes: tipOpacity crash + CPU/uptime stat fixes"; desc: "GuidePopup no longer crashes on non-About tabs, CPU shows real values, uptime format cleaned up"; icon: "󰒃"; clr: "red"; detail: "CRITICAL FIX: GuidePopup tipOpacity fatal error — Behavior on tipOpacity was accidentally added to ALL 7 tabs but only About tab declares tipOpacity. Every non-About tab crashed on load. Removed from all tabs except About. FIXED: CPU stat stuck at 0% — broken awk ternary syntax (s>100?100:s → (s>100?100:s)), swapped to top -bn1 -d 0.1 with ps -eo pcpu fallback. FIXED: sysUptime stuck at 'Loading...' — now populated by resource monitor process. FIXED: Uptime format changed from '2 hours, 15 minutes' to '2h 15m' via sed transform." }
                            ListElement { version: "v1.7.83"; title: "GuidePopup System tab redesign — rotating tips + quick stats"; desc: "New rotating tip cards with SVG lightbulb icon, live CPU/RAM/Disk/Uptime stats, glass-morphism styling"; icon: "󰋼"; clr: "purple"; detail: "NEW: GuidePopup System tab — rotating tips display with custom SVG lightbulb icon (tip_icon.svg), glass-morphism card with blur background, glow orb, and pulse animation. Tips cycle every 10 seconds with fade transitions (fade-out → new tip → fade-in), click to advance immediately. NEW: Quick Stats card below tips — live CPU%, RAM (used/total + %), Disk (used/total + %), Uptime in Xh Ym format. Resource monitor fetches live stats every 5s while System tab is active. CHANGED: All rotating tip code (array, timers, Behavior, Text) removed from About tab — About tab now shows only link cards + version info." }
                            ListElement { version: "v1.7.78"; title: "Network sounds: WAV → MP3 + organized into subfolders"; desc: "Network audio shrunk from 2.1MB to 220KB, sorted into connection/power/navigation subdirectories"; icon: "󰋼"; clr: "blue"; detail: "CHANGED: All network sounds (connect, disconnect, error, power_on, power_off, switch) converted from WAV to MP3 — 2.1MB down to ~220KB total, identical playback quality. CHANGED: Sounds reorganized into subfolders inside sounds/network/: connection/ (connect, disconnect, error), power/ (power_on, power_off), navigation/ (switch). CLEANUP: Old network/sounds/ WAV files removed on update." }
                            ListElement { version: "v1.7.77"; title: "Fix: old MP3 files cleaned up on update — no more duplicates"; desc: "Stale sound files from previous versions now properly removed"; icon: "󰋼"; clr: "blue"; detail: "FIXED: Old MP3 sound files from previous versions were left behind after the v1.7.73 reorganization. battery/*.mp3, sounds/plug-in.mp3, sounds/un-plug.mp3, updater/update-notification.mp3, and music/Ariana Grande - Bye.mp3 were duplicated at their old locations. Now explicitly cleaned on every update." }
                            ListElement { version: "v1.7.76"; title: "Security hardening — input validation & path sanitization"; desc: "Patched potential command injection in audio_autoswitch, path traversal in settings_watcher"; icon: "󰒃"; clr: "red"; detail: "SECURITY: Hardened shell script input validation across audio_autoswitch.sh and usb_sound.sh — prevents potential command injection via malformed USB device descriptors. SECURITY: Patched path traversal vector in settings_watcher.sh — sanitized template variable substitution to prevent arbitrary file read." }
                            ListElement { version: "v1.7.75"; title: "Security: hardened temp file handling & udev cleanup"; desc: "Migrated to mktemp, restricted udev rule removal to owner-only"; icon: "󰒃"; clr: "red"; detail: "SECURITY: Fixed unsafe temporary file handling in install.sh and restore.sh — migrated from predictable /tmp paths to mktemp-based random directories. SECURITY: Patched potential privilege escalation in udev rule cleanup — restricted rule removal to owner-only operations." }
                            ListElement { version: "v1.7.74"; title: "CRITICAL: swayidle 66min suspend fix — black screen on unlock resolved"; desc: "DPMS off replaced with systemctl suspend, after-resume now fires correctly"; icon: "󰒃"; clr: "red"; detail: "CRITICAL FIX: swayidle 66-min timeout used 'hyprctl dispatch dpms off' instead of 'systemctl suspend'. after-resume in swayidle only fires on system resume (from suspend), NOT on user activity after DPMS off. So when DPMS off turned the display off and the user interacted, the display stayed black. Even unlocking blindly left a black screen — force restart was the only escape. FIXED: timeout 4000 now runs 'systemctl suspend'. System actually suspends at 66 min, after-resume fires correctly on wake, display turns back on with Lock.qml password prompt." }
                            ListElement { version: "v1.7.73"; title: "MP3 reorganization — all sounds under quickshell/sounds/ subfolders"; desc: "Battery, USB, updater, and music MP3s now organized in dedicated subfolders"; icon: "󰋼"; clr: "blue"; detail: "CHANGED: All sound MP3s reorganized into quickshell/sounds/{battery,usb,updater,music}/ subfolders for consistency. Script paths updated in 6 files (battery_fetch.sh, check_battery.sh, usb_sound.sh, update_notifier.sh, music_control.sh, lock.sh). Old empty battery/ and music/ directories removed (scripts/QML stay in original locations)." }
                            ListElement { version: "v1.7.72"; title: "Fix: 'Imperative' text replaced with 'WifeRice' in GuideAbout"; desc: "Branding fix — WifeRice name now correct everywhere"; icon: "󰋼"; clr: "blue"; detail: "FIXED: The GuideAbout section displayed 'Imperative' instead of 'WifeRice'. Now shows the correct project name." }
                            ListElement { version: "v1.7.71"; title: "Cumulative fixes: kitty crash, sound files shipped, systemctl restart"; desc: "hyprctl reload detached so terminal survives, MP3s in repo, no more stale process instances"; icon: "󰒃"; clr: "red"; detail: "FIXED: Kitty no longer crashes during update — hyprctl reload runs detached after script exits. FIXED: USB sound MP3 files now ship in repo (Hyprland/sounds/) instead of generated once by ffmpeg. FIXED: install.sh uses systemctl --user restart instead of kill $pids — prevents stale duplicate audio_autoswitch instances. FIXED: settings_watcher.sh --compile reload also detached." }
                            ListElement { version: "v1.7.70"; title: "Fix: Ctrl+Left/Right works + USB dedup + updates actually overwrite"; desc: "keybinding added to template so it survives recompile, USB 1s dedup, install.sh defaults flipped to overwrite"; icon: "󰒃"; clr: "red"; detail: "CRITICAL FIX: Ctrl+Left/Right workspace navigation never worked — was only in generated keybindings.conf, NOT in keybinds.conf.template. Wiped on every settings_watcher --compile. Now in the template. FIXED: USB sounds played multiple times per plug — udev fires multiple events for one action. 1s dedup added to usb_sound.sh. CHANGED: install.sh defaults flipped — Enter=overwrite (was 'keep'). Updates actually reach users now. NEW: AI Rule 11 — 'When we ship an update, the user gets it'." }
                            ListElement { version: "v1.7.69"; title: "Fix: USB sounds — built into audio_autoswitch.sh (no new service)"; desc: "uses udevadm monitor in background fork, same method as battery sounds"; icon: "󰓃"; clr: "purple"; detail: "CRITICAL: USB sounds still not reaching users after v1.7.68 — the udev RUN → su → pw-play chain was fundamentally unreliable (PipeWire not accessible from udev context). COMPLETELY REPLACED: no separate service, no separate script. USB udev monitor is now built directly into audio_autoswitch.sh as a background fork using udevadm monitor --property (no root). Same method as battery sounds (battery_fetch.sh) — user process with full PipeWire access. Old 99-usb-sound.rules auto-removed on update." }
                            ListElement { version: "v1.7.68"; title: "Fix: USB sounds not playing — udev rules relaxed + diagnostics added"; desc: "broader device matching, --test/--check flags, install errors no longer hidden"; icon: "󰓃"; clr: "purple"; detail: "FIXED: USB/device sounds not reaching users — udev rules were too restrictive. Block rule now uses ENV{ID_BUS}=usb instead of ID_FS_USAGE=filesystem (catches all USB partitions reliably). USB device rule no longer requires ENV{ID_MODEL}=?* (some devices lack model strings). NEW: usb_sound.sh --test plays both chimes for manual verification. NEW: usb_sound.sh --check shows diagnostics (file paths, PipeWire, boot guard). FIXED: udev rule install errors now logged to INSTALL_LOG with visible warning." }
                            ListElement { version: "v1.7.67"; title: "Fix: settings_watcher errors no longer silently swallowed"; desc: "compile failures now logged with warning instead of hidden by '2>/dev/null'"; icon: "󰒃"; clr: "red"; detail: "FIXED: settings_watcher.sh --compile was guarded with '2>/dev/null || true' — if monitors.conf generation failed, the error vanished and Hyprland fell back to 1.5 scale. Now errors go to INSTALL_LOG and a yellow warning replaces the green checkmark when compile fails." }
                            ListElement { version: "v1.7.66"; title: "USB/Device Plug/Unplug Notification Sounds — Chime In/Out"; desc: "Audible feedback when USB drives, mice, keyboards, or headphones connect/disconnect"; icon: "󰓃"; clr: "purple"; detail: "NEW: USB/device notification sounds — ascending two-note chime on plug, descending on unplug. NEW: udev rules for USB storage (flash drives, external HDDs via ENV{ID_FS_USAGE}=filesystem) and general USB devices (mice, keyboards, headsets via ENV{ID_VENDOR_ID}=!1d6b). NEW: Headphone/Bluetooth plug/unplug sounds via audio_autoswitch.sh pactl subscriber integration. NEW: usb_sound.sh — rate-limited (2s cooldown) with boot-time protection (60s guard), uses pw-play with paplay fallback. FIXED: Safe udev exec via su with XDG_RUNTIME_DIR set for PipeWire connectivity. Sounds generated at install time using ffmpeg (no binary blobs in repo)." }
                            ListElement { version: "v1.7.65"; title: "swayimg + MIME Fixes + Ctrl+Left/Right Workspace Navigation"; desc: "New image viewer with censor keys, folder association fixed, keyboard workspace switching"; icon: "󰋼"; clr: "purple"; detail: "NEW: swayimg — lightweight Wayland-native image viewer with keyboard pixelation/blur censor effects. NEW: MIME associations — Thunar for folders (fixes Discord/Vencord 'Open folder' opening Gwenview), Gwenview for images (editing). NEW: Ctrl+Left/Right switches workspaces (no interference with Super+1-10 or swipe gestures). FIXED: restore.sh version made dynamic (was stuck at v1.7.59, reads from installed state file). FIXED: Proton VPN package removed from install.sh (not found in AUR)." }
                            ListElement { version: "v1.7.64"; title: "QuickShell package fix — use quickshell-git (AUR) instead of quickshell (official repo)"; desc: "Official repo 'quickshell' conflicts with AUR 'quickshell-git' — switching to git version avoids conflicts for existing users"; icon: "󰨞"; clr: "blue"; detail: "FIXED: QuickShell package conflict during install — official repo 'quickshell' (v0.3.0) conflicted with AUR 'quickshell-git' (v0.2.0.r148). The git version has the latest commits and is what we develop against. Now installs 'quickshell-git' via yay instead. Same fix as v1.7.63 for VS Code." }
                            ListElement { version: "v1.7.63"; title: "VS Code package fix — prefer AUR visual-studio-code-bin over repo code"; desc: "Replaced official repo 'code' with AUR 'visual-studio-code-bin' to avoid conflict with users who already have the AUR version"; icon: "󰨞"; clr: "blue"; detail: "FIXED: VS Code package conflict during install — official repo 'code' conflicted with AUR 'visual-studio-code-bin'. Now installs 'visual-studio-code-bin' (Microsoft's official binary) via yay instead. Users who already have the AUR version will no longer see the conflict error." }
                            ListElement { version: "v1.7.62"; title: "Final Polish — Removed Invalid touchpad accel_profile + Full Audit Complete"; desc: "Hyprland config error fixed, all 64 audit issues + scale cascade + gaming settings done"; icon: "󰒃"; clr: "red"; detail: "FIXED: Hyprland config error 'input:touchpad:accel_profile does not exist' — accel_profile is only valid in the main input{} block (which already applies to both mouse and touchpad). Removed the invalid touchpad sub-block line. COMPLETE: All 64 audit issues from v1.7.59 fixed, scale cascade resolved with PID lock, gaming mouse acceleration disabled, broken fonts replaced. This is the final polish commit." }
                            ListElement { version: "v1.7.61"; title: "Scale Bug Fix + Gaming Mouse/Touchpad — PID Lock Prevents Watcher Cascade"; desc: "Settings watcher cascade loop caused Hyprland scale to reset to 1.5; mouse + touchpad acceleration now disabled for gamers"; icon: "󰒃"; clr: "red"; detail: "CRITICAL FIX: Scale bug came back (v1.7.50 regression) — settings_watcher.sh --compile triggered hyprctl reload, which spawned a SECOND watcher instance. Two watchers raced, creating a cascade of hyprctl reloads that reset the scale to ~1.5 despite monitors.conf saying scale=1. FIXED: Added PID lock file to settings_watcher.sh — prevents duplicate instances. --compile mode kills any old watcher before running. The hyprctl keyword monitor scale-forcing code already existed but was defeated by the cascade. Also: replaced broken ttf-ms-win11-auto with ttf-liberation + ttf-dejavu (confirmed via v1.7.59 install log). NEW: Disabled mouse acceleration (accel_profile = flat) for precise gaming aim. NEW: Disabled touchpad acceleration (touchpad accel_profile = flat)." }
                            ListElement { version: "v1.7.60"; title: "Font Fix — Replaced Broken ttf-ms-win11-auto with Working Open-Source Fonts"; desc: "ttf-ms-win11-auto AUR package fails at build time, replaced with ttf-liberation + ttf-dejavu"; icon: "󰋼"; clr: "blue"; detail: "FIXED: ttf-ms-win11-auto AUR package consistently fails to build (symlink error, exit status 4) — Microsoft blocks auto-downloads. REPLACED with ttf-liberation (metric-compatible Arial/Times/Courier New replacements) and ttf-dejavu (wider Unicode coverage). v1.7.59 installation log confirmed the failure — now fixed for all future installs." }
                            ListElement { version: "v1.7.59"; title: "Full Audit Fix — install.sh Overhaul, Webhook Security, and 50+ Bug Fixes"; desc: "64 audit issues fixed: install.sh filled with real packages, Discord tokens extracted, shell scripts hardened, configs synced"; icon: "󰒃"; clr: "red"; detail: "COMPREHENSIVE AUDIT FIX: All 64 issues from full code audit resolved. CHANGED: install.sh — 12 empty steps filled with real package installs (GPU drivers, PipeWire, yay, fonts, apps), set -e hardened with || true guards, sudo keepalive properly killed on EXIT trap, pacman -Sy added before first install, TELEMETRY_OPT_OUT renamed to TELEMETRY_ENABLED, SDDM theme.conf uses single write, rsync --delete guarded against empty source (prevents QML wipe on failed clone), 2>/dev/null removed from all pacman calls, summary now accurately reflects installed packages. SECURITY: Discord webhook URLs extracted from 14 scripts into shared .telemetry_config — no more hardcoded tokens in source code. FIXED: lock.sh division by zero + empty PID kill (killed all processes), fetch_workspaces.sh JSON injection via unquoted heredoc, restore.sh version hardcoded to 1.7.20 (now dynamic), tilde → $HOME across all scripts, update_notifier.sh now handles notification clicks, volume_listener.sh locale-independent parsing, sudo commands guarded with timeout to prevent hangs, theme_profiles.sh wallpaper apply uses awww directly. CLEANUP: hypridle.conf no longer deployed (swayidle is default), settings.json uiScale made integer consistent, missing shebangs added." }
                            ListElement { version: "v1.7.58"; title: "CRITICAL FIX: Thunar Thumbnails Not Showing — Missing Packages Finally Installed"; desc: "tumbler, ffmpegthumbnailer, webp-pixbuf-loader, libopenraw, libgepub were never actually installed by install.sh"; icon: "󰒃"; clr: "red"; detail: "CRITICAL FIX: Thunar and file managers showed no thumbnails — the install.sh claimed 'Thumbnails: ffmpegthumbnailer, tumbler, libopenraw, libgepub, webp' in its summary but NEVER actually installed these packages. All five thumbnail packages were missing from every pacman command. FIXED: Added tumbler (thumbnail service), ffmpegthumbnailer (video thumbnails), webp-pixbuf-loader (WebP images), libopenraw (RAW camera photos), and libgepub (EPUB books) to step 5 package install. FIXED: Freedesktop thumbnail cache cleared on install so thumbnails regenerate fresh." }
                            ListElement { version: "v1.7.57"; title: "NTFS External Drive Fix — Auto-Mount Windows/USB Drives in Thunar"; desc: "Blacklists kernel ntfs3 driver, uses ntfs-3g FUSE instead — NTFS drives mount without errors"; icon: "󰋼"; clr: "blue"; detail: "FIXED: External NTFS hard drives (Windows USB drives, external HDDs) failed to mount in Thunar with 'wrong fs type' error. Root cause: kernel ntfs3 driver loaded and interfered with ntfs-3g FUSE driver — ntfs3 lacks codepage/NLS support. FIXED: install.sh now auto-installs ntfs-3g, blacklists kernel ntfs3 driver via /etc/modprobe.d/blacklist-ntfs3.conf, and unloads ntfs3 module immediately — no reboot needed. Drives appear in Thunar → Devices sidebar automatically." }
                            ListElement { version: "v1.7.55"; title: "CRITICAL: settings.json Still Referenced hypridle — No Idle Daemon Running"; desc: "autostart.conf regenerated with hypridle on compile, but hypridle was removed"; icon: "󰒃"; clr: "red"; detail: "CRITICAL FIX: settings.json still referenced hypridle — autostart.conf was regenerated with hypridle on compile, but hypridle was removed. Result: NO idle daemon running — lid close suspended without locking. FIXED: settings.json and default_settings.json now use swayidle command directly. FIXED: DOTS_VERSION bumped so users actually receive the update notification." }
                            ListElement { version: "v1.7.54"; title: "hypridle → swayidle — Fix ABI Crash, Lock on Lid Close"; desc: "hypridle v0.1.7 segfaults with sdbus-cpp 2.3.0, replaced with swayidle"; icon: "󰒃"; clr: "red"; detail: "FIXED: hypridle crash — replaced with swayidle (hypridle v0.1.7 segfaults with sdbus-cpp 2.3.0 ABI mismatch). FIXED: No more polkit auth prompts on lid close — Lock.qml runs directly via WlSessionLock Wayland protocol. FIXED: lock-session → lock-sessions (plural) in hypridle.conf (now OBSOLETE). CHANGED: hypridle removed — swayidle is now the default idle daemon with 2-stage timeout (33min lock, 66min DPMS off)." }
                            ListElement { version: "v1.7.53"; title: "TopBar Streamlined — SVG Icons, Toggles Removed, Gaming Mode Deleted"; desc: "All emoji toggles replaced with SVG icons, 7 buttons removed from TopBar + sidebar"; icon: "󰋼"; clr: "blue"; detail: "CHANGED: All toggle buttons (Caffeine, Night Light, DND, Power Profile, Gaming, VPN) removed from TopBar — streamlined for coders. CHANGED: System Monitor and Quick Notes buttons removed from left sidebar. REMOVED: Gaming Mode — entire feature removed, not designed for developer workflow. NEW: SVG Icon engine — all previous emoji icons replaced with crisp PathSvg vector icons throughout TopBar." }
                            ListElement { version: "v1.7.52"; title: "10 New Features — Quick Toggles & Popups"; desc: "Caffeine, Night Light, DND, Power Profile, Gaming Mode, VPN, Quick Notes, System Monitor, Workspace Overview, Theme Profiles"; icon: "󰋼"; clr: "purple"; detail: "ADDED: Caffeine Toggle (☕) — prevents system suspend via systemd-inhibit. ADDED: Night Light (🌙) — wlsunset blue light filter 3500K. ADDED: Do Not Disturb (🔇) — suppress all notifications. ADDED: Power Profile Switcher (⚡/🚀/🔋) — cycle balanced/performance/power-saver. ADDED: Gaming Mode (🎮) — disables compositor, performance gov, DND+Caffeine. ADDED: VPN Quick Manager (🔒) — toggle WireGuard. ADDED: Quick Notes (Super+Shift+N) — floating text editor saves to ~/Notes.md. ADDED: System Monitor (Super+Shift+M) — live CPU/RAM/disk bars. ADDED: Workspace Overview (Super+Shift+W) — visual workspace grid. ADDED: Theme Profiles — save/restore state via theme_profiles.sh. All toggles available in TopBar right-side pill." }
                            ListElement { version: "v1.7.51"; title: "Website/Dotfiles Audit — 8 Inconsistencies Fixed"; desc: "updates.json unstick, settings.json policy corrected, stars badge fixed, changelog caught up"; icon: "󰋼"; clr: "purple"; detail: "FIXED: Website/dotfiles audit — 8 misinformation/inconsistency issues resolved across both repos. FIXED: updates.json latest_version was stuck at 1.7.28 — users missed 8 critical releases (quoting crash, scale fix, settings_watcher). FIXED: Website changelog was 8 versions behind — stopped at v1.7.42, now complete. FIXED: Website docs (faq.html, install-dots.html) said settings.json is never touched — FALSE. Now correctly documents updated-every-release policy with backup safety net. FIXED: install.sh header comment used old broken double-quote one-liner syntax. FIXED: home.html fake 3.8k stars badge replaced with dynamic shields.io badge. FIXED: All website Last updated dates refreshed." }
                            ListElement { version: "v1.7.50"; title: "Scale Now Applies Correctly — hyprctl keyword monitor After Reload"; desc: "settings_watcher now calls hyprctl keyword monitor directly after reload to force scale changes"; icon: "󰒃"; clr: "green"; detail: "CRITICAL FIX: hyprctl reload does NOT apply monitor scale changes. settings_watcher now runs hyprctl keyword monitor for each monitor from settings.json after reload, forcing the correct scale to take effect immediately. Fixes the 'everything oversized' bug (Chrome, kitty, fonts, notifications too big after update). install.sh now also calls settings_watcher.sh --compile after scripts deploy." }
                            ListElement { version: "v1.7.49"; title: "CRITICAL: Fix Update Crash + Scale Not Applied + settings.json Overwrite"; desc: "Fixed update button quoting crash, settings_watcher not called after deploy, settings.json always overwritten"; icon: "󰒃"; clr: "red"; detail: "CRITICAL FIX 1: GuidePopup.qml update buttons used double quotes for bash -c \"$(curl ...)\" — install.sh double quotes terminated the string prematurely, crashing the terminal. Now uses single quotes + eval. CRITICAL FIX 2: install.sh never called settings_watcher.sh --compile after deploying settings.json — monitors.conf stayed stale, runtime scale was stuck at old value (e.g. 1.50) even though settings.json said scale 1. Now calls settings_watcher.sh --compile after scripts deploy to regenerate monitors.conf and reload Hyprland. ALSO: settings.json always overwritten with auto-detected monitor name. Backup to /tmp/. Final validation restores backup if invalid." }
