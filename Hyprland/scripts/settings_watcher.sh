...
compile_settings() {
    ...
    if [[ $(jq -r 'if (if type == "object" and has("openGuideAtStartup") then .openGuideAtStartup else true end) then "yes" else "no" end' "$SETTINGS_FILE") == "yes" ]]; then
        echo "exec-once = bash -c 'sleep 1 && $HOME/.config/hypr/scripts/qs_manager.sh toggle guide'" >> "$AUTOSTART_CONF"
    fi
    ...
    if [ "$OLD_HASH" != "$NEW_HASH" ]; then
        # Detach reload so GUI terminal (kitty) survives hyprctl reload
        ( sleep 0.3; hyprctl reload 2>/dev/null; jq -r '.monitors[]? | "hyprctl keyword monitor \(.name),\(.resW)x\(.resH)@\(.rate),\(.x)x\(.y),\(.scale)"' "$SETTINGS_FILE" 2>/dev/null | bash || true; ) &
    fi
}
